...
import numpy as np
from transformers import AutoTokenizer, BertTokenizer, BertModel, AutoConfig
...
conn = sqlite3.connect('optuna_p2.db')
...
# BertAveragePoolingClassifier: Costum classifier as required in part 2
class MeanPoolingBert(AutoModelForSequenceClassification):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        # Stack all hidden states and apply mean pooling
        hidden_states = torch.stack(hidden_states, dim=0)
        pooled_output = self.mean_pooling(hidden_states).squeeze()

        logits = self.classifier(pooled_output)

        loss = None
        if labels is not None:
            loss = super().compute_loss(outputs.logits, labels)
...
model = MeanPoolingBert.from_pretrained('bert-base-uncased', config=config)
# model.dropout.p = 0.2

metric_name = "pearson" if task == "stsb" else "matthews_correlation" if task == "cola" else "accuracy"
model_name = 'MeanPoolingBert'
...
def model_init():
    config = AutoConfig.from_pretrained('bert-base-uncased')
    return MeanPoolingBert.from_pretrained('bert-base-uncased', config=config)
...
optuna.visualization.plot_optimization_history(study).write_image('p2_optuna_optimization_history.png')
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("p2_loss_over_time.png")
...
plt.legend()
plt.savefig("p2_mcc_over_time.png")
